...
int main(int argc, char** argv)
{
	...
	/*
	* Generate AUto tests to test the accuracy of the program
	*/
	vector<bool>tests = { 1,1,1,0,1,1,1 };
	vector<bool>out = vector<bool>(tests.size(),false);
	int test = tests.size();
	for (int i = 0; i < test; i++)
	{
		...
		std::string path2 = "./assets/ontrack/" + s + ".jpeg";
		...
		//Debug
		Mat draw__temp = image_lines.clone();
		cv::line(draw__temp, Point(x_center, y_center), Point(0, 0), Scalar(255, 255, 255), 2);
		imshow("Center vs lines", draw__temp);
		//waitKey(0);
	...
	cout << "----------------------------------- out of the tests -----------------------------------" << endl;
	cout << "answer\tout\tcase" << endl;
	for (int i = 0; i < test; i++)
		cout << tests[i] << "\t" << out[i] << "\t" << ((out[i] == tests[i]) ? "AC" : "Worng") << endl;
	return 0;
}
