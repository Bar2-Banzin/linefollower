...
void car_on_line(bool& on_line, double x_car_front, double  y_car_front, double  x_car_back, double y_car_back, Mat& lines_matrix,  int threshold, int thickness ,double perpend_factor,double front_factor) {
    /**
...
Mat check=cv::Mat::zeros(cv::Size(size_j, size_i), CV_8UC1);
	for (int i = 0;i < windo_size_y;i++) {
		...
		for (int j = 0;j < windo_size_x / 2;j++) {
			point_x = floor(base_x + j * per_unit_vector_x);
			point_y = floor(base_y + j * per_unit_vector_y);
			if (point_x < 0 || point_y < 0 || point_x >= lines_matrix.cols || point_y >= lines_matrix.rows)
				continue;
			if ((int)check.at<uchar>(point_y, point_x) == 1)
				continue;
			auto scaler = (int)temp_matrix.at<uchar>(point_y, point_x);
			count2 += ((int)scaler != 0) ? 1 : 0;
			check.at<uchar>(point_y, point_x) = (int)1;
	...
	on_line = count2 > threshold*distance_between_2_centers* thickness;
    //Car Center
